...
- Errors（至少需要）：
  - `TOKEN_INVALID` / `TOKEN_EXPIRED`
  - `VALIDATION_ERROR`
  - `HOME_BASE_UPDATE_LIMITED`（自然日 0 点重置，1 天最多修改 1 次）
...
- Request body（后端 main 实现口径）：
  ...
  - `radiusM?: number`（可见半径，单位：米；默认 1000）
...
- Query：
  - `atLat=<number>`（optional；用于按发布者 radiusM 判断可见，建议尽量传）
  - `atLng=<number>`（optional）
- Response：
  - `posts: localFeedPostItem[]`（后端会做有效期 + 半径 + 置顶排序等裁剪）
